...
class NetworkManager:
    ...
    async def wait_for_ip(self, namespace, interface, mac, timeout=20):
        """Polls the namespace for IPv4 and IPv6 assignment."""
        start = time.time()
        ...
        while time.time() - start < timeout:
            try:
                ...
                # If we have at least IPv4, consider success
                if ipv4_only:
                    logger.info(
    ...
    async def create_client(self, i):
        """Creates a single macvlan client within a namespace."""
        ns = f"ns{i}"
        macvlan = f"macvlan{i}"
        ...
        try:
            ...
            # 2. Create macvlan link and move to namespace
            await run_cmd(
            ...
            # Retry logic for IP acquisition
            for attempt in range(4):
                if await self.wait_for_ip(ns, macvlan, mac):
                    self.count += 1

                    # 5. Setup IPv6 and DNS
                    ...
                    dns_cmd = (
                        f'echo -e "nameserver 8.8.8.8\\nnameserver 1.1.1.1" | '
                        f'sudo tee /etc/netns/{ns}/resolv.conf'
                    )
                    await run_cmd(dns_cmd)
                    return

                logger.warning(f"{ns} retrying IP acquisition ({attempt + 1}/4)...")
                await asyncio.sleep(1)

            self.failure_messages[ns] = "DHCP Timeout after 4 attempts"
    ...
    async def create_clients(self, count):
        """Orchestrates parallel creation of multiple clients."""
        ...
        # This triggers the detailed table in your log
    ...
    def display_client_table(self):
        """Formats and logs the detailed PrettyTable summary."""
        table = PrettyTable()
    ...
    async def cleanup(self):
        """Cleans up all network namespaces and configuration files."""
