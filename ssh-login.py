...
domain='https://piranha-int.tau.dev-charter.net'
email='robot-testing-user@charter.com'
passwd = 'Robot@Testing'

sshpassword="Charter123"
time_out=3600
...
def enable_ssh(token, state):
    ...
    print("Enabling ssh for the node "+nodeid+" on the domain - "+domain+" for next "+str(time_out)+" seconds") if int(state) else print("Disabling ssh for the node"+nodeid+"on the domain - "+domain)
    ...
    payload_dict = {"kvConfigs": [
        { "module": "SSHM", "key": "sshAccessControl", "value": "true" if int(state) else "false"},
    ...
    url = domain+'/api/Customers/'+customerid+'/locations/'+locationid+'/nodes/'+nodeid+'/kvConfigs'
    headers = {'Content-Type': 'application/json', 'Authorization':token}
    ...
    try:
        ...
        print("read error - wanIp")

    try:
        lanip=nod_info["ip"]
        print(" ssh operator@"+lanip)
    except:
        print("read error - lanip")
    ...
    try:
        ...
        print(" read error - ipv6 addesses")
    try:
        for name in nod_info["ipv6"]:
            splitstring=name.split("/",1)
            print(" ssh operator@"+splitstring[0])
...
if __name__ == "__main__":
    ...
    (opts, val) = getopt.getopt(sys.argv[1:], "i:n:e:h",
                              ["nodeid=","noc=","enable=","help"])
    # print(opts,val)
